...
struct thr_pool *g_thread_pool = 0;
...
static int create_worker(thr_pool_t *pool) {
    ...
    log("in attr:%ld 0:%d", pool->pool_attr.__sig, pool->pool_attr.__opaque[0]);
    (void)pthread_sigmask(SIG_SETMASK, &fillset, &oset);
    error = pthread_create(&pool->th, &pool->pool_attr, worker_thread, pool);
...
static void job_cleanup(thr_pool_t *pool) {
    ...
    if (pool->pool_flags & POOL_WAIT) notify_waiters(pool);
}
static void *worker_thread(void *arg) {
    ...
    for (;;) {
        ...
        pool->pool_idle--;
        if (pool->pool_flags & POOL_DESTROY) break;
        if ((job = pool->pool_head) != NULL) {
            timedout = 0;
            func = job->job_func;
            arg = job->job_arg;
            pool->pool_head = job->job_next;
            if (job == pool->pool_tail) pool->pool_tail = NULL;
...
void thr_pool_destroy(thr_pool_t *pool) {
     ...
     */
    (void)pthread_mutex_lock(&thr_pool_lock);
    if (thr_pools == pool) thr_pools = pool->pool_forw;
    if (thr_pools == pool)
        thr_pools = NULL;
    else {
